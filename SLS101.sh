#!/bin/bash
echo "SuperLazyLaunch..."
sudo su -c 'mate-terminal --geometry 48x48 -x sudo kalitorify -t' | sudo su -c 'mate-terminal --full-screen /home/usr/bin/cmatrix -x cmatrix' & sudo su -c 'mate-terminal --working-directory /root/lscript set --active --full-screen --zoom 2.0 -x bash l'
...
## Hit small L key (l) when the script is done loading Kalitorify the Lazy script(if installed) is now opened in your root terminal window while kalitorify and cmatrix is running in the background.
# Q=ExitsTheCmatrix and you can now hit enter or ctrl+c to shut it down. Or you can un-comment or delete the read command on line 4. ver.SLS1.01.
# You will be prompted, whether you want to keep Kalitorify running or not, when closing the LazyScript.
## Adjust your terminal window opacity so that you can see the data in the background. Ver.1.02 will hopefully have fully implemented information-gathering-tools ez2use - and later maybe with UI & executable. Won't touch the opacity functions, that is user preferance.
